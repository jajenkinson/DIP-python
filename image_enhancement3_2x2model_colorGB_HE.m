%   call: image_enhancement3_2x2modelRGBandHE.m
%   from: image_enhancement3_2x2modelRGBandHE.m
%   opengl('save', 'software');
%
%   Demo code for gradient-based HE
%   Code for John for practice.
%
%	Artyom M. Grigoryan, June 1, 2025
 ...
 if kk==1
    ...
    stitle=sprintf('The original [EMEC %4.2f]',E1c);
    h_t=title(stitle);
    set(h_t,'FontName','Times','FontSize',10); 
    pause(1)
 end
...
disp('  Visually observe the effect of parameter of the GB-HE' );
disp('  Results are stable (!)' );

a_step=0.1;
aa=1.1:a_step:2; nn=length(aa);
EMEC=zeros(1,nn);  % to save all values of EMEC
...
for a2=1.1:a_step:2
   ...
   %E3=eme2(X_eq2,L5,L7);    
    ...
    pause(1)
    s_title=sprintf('Enhanced image [EMEC=%4.2f]',E3c); 
    h_t1=title(s_title);
    set(h_t1,'FontName','Times','FontSize',10); 
...
% --- Function EMEC(a2) monotonic desriasing (as should)
% figure;
% plot(EMEC)
